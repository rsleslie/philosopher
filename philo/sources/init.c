...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/02 17:58:02 by rleslie-          #+#    #+#             */
/*   Updated: 2023/07/02 18:00:14 by rleslie-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_threads(t_data *data, t_philo *philo)
{
	...
	i = -1;
	th = malloc(sizeof(pthread_t) * (data->n_philosopher + 1));
	while (++i < data->n_philosopher)
	{
		philo[i].t_begin = time_begin();
		philo[i].last_meals = philo[i].t_begin;
	}
	i = -1;
	while (++i < data->n_philosopher)
	{
		if (pthread_create(&th[i], NULL, &routine, &philo[i]) != 0)
			return (1);
		if (i + 1 == data->n_philosopher)
		{
			if (pthread_create(&th[i + 1], NULL, &observer, philo) != 0)
				return (1);
		}
	}
	i = -1;
	while (++i < (data->n_philosopher + 1))
